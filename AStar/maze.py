...
class Maze:
    """Maze class with methods including constructor"""
    ...
    def get_neighbors(self, node):
        """finding nearby neighbors for precalculation of moves"""
        neighbors = []
        x, y, orientation = node.x, node.y, node.orientation

        if orientation == 'vertical':
            moves = [
                (1, 0, 'horizontal_h', 'right'),
                (-1, 0, 'horizontal_h', 'left'),
                (0, 1, 'horizontal_v', 'up'),
                (0, -1, 'horizontal_v', 'down')
            ]
            for dx, dy, new_orientation, move in moves:
                nx, ny = x + dx, y + dy
                if new_orientation == 'horizontal_h' and \
                   self.is_valid(nx, y) and \
                   self.is_valid(nx + 1, y):
                    neighbors.append(Node(nx, y, new_orientation, move, node))
                elif new_orientation == 'horizontal_h' and \
                     self.is_valid(nx, y) and \
                     self.is_valid(nx - 1, y):
                    neighbors.append(Node(nx, y, new_orientation, move, node))
                elif new_orientation == 'horizontal_v' and \
                     self.is_valid(x, ny) and \
                     self.is_valid(x, ny + 1):
            ...
            for dx, dy, new_orientation, move in moves:
                nx, ny = x + dx, y + dy
                if new_orientation == 'vertical' and \
                   self.is_valid(x, ny):
                    neighbors.append(Node(x, ny, new_orientation, move, node))
                elif new_orientation == 'vertical' and \
                     self.is_valid(x, ny):
                    neighbors.append(Node(x, ny, new_orientation, move, node))
                elif new_orientation == 'horizontal_v' and \
                     self.is_valid(nx, y) and self.is_valid(nx + 1, y):
                    neighbors.append(Node(nx, y, new_orientation, move, node))
                elif new_orientation == 'horizontal_v' and \
                     self.is_valid(nx, y) and self.is_valid(nx - 1, y):
                    neighbors.append(Node(nx, y, new_orientation, move, node))
